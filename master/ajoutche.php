...
<html>
<head>
  <title>Ajouter un chef de projet</title>
</head>
<body>
  <h1>Ajouter un chef de projet</h1>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlchefprojet = "INSERT INTO chefprojet (matricule_chefprojet, nom, Prenom, telephone, email, login, motDePass, profil)
   VALUES ('$matricule', '$nom', '$prenom', '$telephone', '$email', '$login', '$motDePasse', '$profil')";
...
if ($conn->query($sqlutilisateur) && $conn->query($sqlchefprojet) && $conn->query($sqlhisto)) {
  header("Location: afichachef.php");
}
  }
  ?>
</body>
